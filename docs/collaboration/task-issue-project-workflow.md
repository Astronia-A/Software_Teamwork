...
[Task Issue 模板](../../.github/ISSUE_TEMPLATE/issue.md) 为唯一模板源；本文档只维护发布流程、编号规则、依赖规则和 Project 同步规则。仓库维护配置见
...
3. 判断任务归属，选择编号前缀。
4. 按 GitHub Issue 模板写完整任务书正文。
5. 补齐 `依赖任务`、`阻塞任务`、`并行任务` 和 `依赖原因`。
6. 逐张校对任务书。
7. 先发布上游任务，再发布下游任务。
8. 等待 Task Issue Sync 自动加入 Project、同步字段和更新 `Project sync`。
9. 回填依赖 issue 编号，确认 `Project sync` 为 `synced`。
...
- 使用 [Task Issue 模板](../../.github/ISSUE_TEMPLATE/issue.md)。
...
5. 测试交付类 `T-*` 按被测对象依赖关系插入发布顺序；测试发现但需要开发处理的问题，按对应开发组发布。
